<script lang="ts">
  ...
  onMount(() => {
    if (titleEl)
      animate(titleEl, { scale: [0.4, 1], opacity: [0, 1], duration: 700, ease: 'outBack' });
    if (confettiEl) {
    ...
    (async () => {
      ...
      if (document.querySelectorAll('[data-lb-row]').length > 0) {
        tl.add(
          '[data-lb-row]',
          { opacity: [0, 1], translateX: [-20, 0], duration: 400, delay: stagger(50) },
          500
        );
      }
...
<section
  class="pointer-events-auto absolute inset-0 z-40 flex flex-col items-center justify-center gap-6 overflow-hidden bg-black/85"
>
  <div bind:this={confettiEl} class="pointer-events-none absolute inset-0">
    {#each Array(40) as _, i}
      <span
        class="absolute top-1/3 left-1/2 inline-block h-3 w-3 rounded"
  ...
  <div class="z-10 flex flex-col items-center gap-2">
    <h2 bind:this={titleEl} class="px-6 text-center text-6xl font-black md:text-7xl">
      {winnerLabel}
    </h2>
    <p class="font-mono text-xl opacity-80 md:text-2xl">
      {r?.winner === 'draw'
        ? "Time's up!"
        : `Solved in ${((r?.durationMs ?? 0) / 1000).toFixed(2)}s`}
    </p>
  </div>

  <div class="z-10 mt-2 flex gap-4">
    <Button size="lg" onclick={rematch}>Rematch</Button>
    <Button size="lg" variant="outline" onclick={newPlayers}>New players</Button>
  </div>

  <div class="z-10 mt-4 w-full max-w-sm px-6" data-lb-card style="opacity: 0;">
    <div class="rounded-2xl border border-white/20 bg-white/10 p-5 shadow-2xl backdrop-blur-md">
      <h3 class="mb-4 text-center text-sm font-bold tracking-widest text-white/70 uppercase">
        Top Times
      </h3>
      <div class="flex flex-col gap-2">
        {#each leaderboard.scores as score, i}
          <div
            class="flex items-center justify-between rounded-lg px-3 py-2 {i + 1 === highlightRank
              ? 'new-score-row border border-white/30'
              : ''}"
            data-lb-row
            style="opacity: 0;"
          >
            <div class="flex items-center gap-3">
              <span class="w-4 text-right font-mono text-sm opacity-50">{i + 1}</span>
              <span class="font-medium {i + 1 === highlightRank ? 'text-white' : 'text-white/90'}"
                >{score.name}</span
              >
            </div>
            <span
              class="font-mono {i + 1 === highlightRank
        ...
        {#if leaderboard.scores.length === 0}
          <p class="py-2 text-center text-sm italic opacity-50">No scores yet</p>
        {/if}
      </div>
    </div>
  </div>
